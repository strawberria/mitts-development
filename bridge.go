...
func (b *Bridge) ExportProject(data string) {
	filename, err := dialog.File().SetStartFile("project.json").Title("Export Project").Filter("JSON File (*.json)", "json").Save()
	if err != nil {
		return
	}
...
func (b *Bridge) ImportProject() string {
	filename, err := dialog.File().Title("Import Project").Filter("JSON File (*.json)", "json").Load()
	if err != nil {
		return err.Error()
	}
